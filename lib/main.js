...
function tokenExchangeHandler({
  ...
  assert.isFunction(authenticateClient, 'authenticateClient');
  assert.isFunction(getClient, 'getClient');
  assert.isFunction(issue, 'issue');
  ...
  return async (req, res) => {
    try {
      ...
      const {accessToken, expiresIn, scope} = await issue({
